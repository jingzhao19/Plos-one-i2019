...
for (i in r) {
  for (j in n)  {
    dcsh=read.csv(paste0("nullarh_arh_r",i,"n",j,"_main.csv"),header=T)
    dcshh=read.csv(paste0("nullarh_arh_r",i,"n",j,"_slice.csv"),header=T)
    dcs=read.csv(paste0("nullarh_ar_r",i,"n",j,"_main.csv"),header=T)
    dcss=read.csv(paste0("nullarh_ar_r",i,"n",j,"_slice.csv"),header=T)
    dt1=read.csv(paste0("nullarh_arh_r",i,"n",j,"_Seq ANOVA for time 1.csv"),header=T)
    dt2=read.csv(paste0("nullarh_arh_r",i,"n",j,"_Seq ANOVA for time 2.csv"),header=T)
    dt3=read.csv(paste0("nullarh_arh_r",i,"n",j,"_Seq ANOVA for time 3.csv"),header=T)
    dt4=read.csv(paste0("nullarh_arh_r",i,"n",j,"_Seq ANOVA for time 4.csv"),header=T)
    dt5=read.csv(paste0("nullarh_arh_r",i,"n",j,"_Seq ANOVA for time 5.csv"),header=T)
    da<-est(dcsh,dcshh,dcs,dcss,dt1,dt2,dt3,dt4,dt5)
    head(da)
    p<-ggplot(da, aes(x=ProbF))+
      geom_histogram(aes(y=0.05*..density..),binwidth=0.05,color="blue", fill="light blue") +
      facet_grid(method1 ~ Effect1)
    p + xlab("P value") + 
      ylab("Proportion") +
      ggsave(paste0("arh_null_r",i,"n", j, ".tiff"),
