...
for (i in r) {
  for (j in n)  {
    dcsh=read.csv(paste0("result_csh_csh_r",i,"n",j,"_main.csv"),header=T)
    dcshh=read.csv(paste0("result_csh_csh_r",i,"n",j,"_slice.csv"),header=T)
    dcs=read.csv(paste0("result_csh_cs_r",i,"n",j,"_main.csv"),header=T)
    dcss=read.csv(paste0("result_csh_cs_r",i,"n",j,"_slice.csv"),header=T)
    dt1=read.csv(paste0("result_csh_anova_r",i,"n",j,"_T1.csv"),header=T)
    dt2=read.csv(paste0("result_csh_anova_r",i,"n",j,"_T2.csv"),header=T)
    dt3=read.csv(paste0("result_csh_anova_r",i,"n",j,"_T3.csv"),header=T)
    dt4=read.csv(paste0("result_csh_anova_r",i,"n",j,"_T4.csv"),header=T)
    dt5=read.csv(paste0("result_csh_anova_r",i,"n",j,"_T5.csv"),header=T)
    ...
    p + xlab("P value") + 
      ylab("Proportion") +
      ggsave(paste0("alter_r",i,"n", j, ".tiff"),
